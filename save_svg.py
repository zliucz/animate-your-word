...
def check_hole(word, font):
    hole = []
    if font == "KaushanScript-Regular":    
        for letter in word :
            if letter in ['A','D','P','R','d','e','o','q','s']:
                hole.extend([0,1])
            elif letter in ['B']:
                hole.extend([0,1,1])
            elif letter in ['K','i','j']:
                hole.extend([0,0])
            else:
                hole.append(0)
    elif font == "Roboto-Light" or font == "Roboto-Bold":
        for letter in word :
            if letter in ['A','D','O','P','Q','R','a','b','d','e','g','o','p','q']:
                hole.extend([0,1])
            elif letter in ['B']:
                hole.extend([0,1,1])
            elif letter in ['i','j']:
        ...
        for letter in word :
            if letter in ['A','D','O','P','Q','a','b','d','e','g','o','p','q']:
                hole.extend([0,1])
            elif letter in ['i','j']:
                hole.extend([0,0])
            else:
...
def save_svg(filename, width, height, shapes, shape_groups,word,font,use_gamma = False, background=None):
    holes=check_hole(word,font)
    # print('len_hole',print(len(holes)))
    # print('len_shape_groups',len(shape_groups))
    ...
    for i, shape_group in enumerate(shape_groups):
        ...
        def add_color(i, shape_color, name):
            ...
            if isinstance(shape_color, pydiffvg.RadialGradient):
                ...
                for j in range(offsets.shape[0]):
                    ...
                    # print('?',255 * c[0])
                    stop.set('stop-opacity', '{}'.format(c[3]))
        if shape_group.fill_color is not None and i==0:
            add_color(i, shape_group.fill_color, 'shape_{}_fill'.format(i))


        if shape_group.stroke_color is not None:
            add_color(i, shape_group.stroke_color, 'shape_{}_stroke'.format(i))
    for i, shape_group in enumerate(shape_groups):
        ...
        for j,id in enumerate(shape_group.shape_ids):
            ...
            if shape_group.fill_color is not None and holes[i]==0:
            ...
            elif   shape_group.fill_color is not None and holes[i]==1:
                    shape_node.set('fill', 'rgb({}, {}, {})'.format(\
